...
const DeleteNotificationModal = ({ onClose, notifications, notification, setNotifications }: DeleteNotificationModalProps) => {
  ...
  const clickHandler: React.MouseEventHandler<HTMLButtonElement> = async () => {
    try {
      ...
      const response = await fetch(`${server}/api/users/notifications/${user._id}`, {
        ...
        body: JSON.stringify({ createdAt: notification.createdAt, message: notification.message, type: notification.type, userId: user._id }),
      });
      ...
      setNotifications(
        notifications.filter((noti) => {
          const isSame =
            new Date(noti.createdAt).getTime() === new Date(notification.createdAt).getTime() &&
