...
export const uploadUserImage = async (file: File, userId: string, token: string): Promise<string> => {
...
export const checkUserImageExists = async (userId: string, token: string): Promise<{ exists: boolean, url?: string }> => {
...
export const uploadBlobAsUserImage = async (blob: Blob, userId: string, token: string): Promise<string> => {
  const file = convertBlobToFile(blob, `${userId}.jpg`);
  return await uploadUserImage(file, userId, token);
};

export const updateUserPhotoIfNecessary = async (oid: string, token: string, server: string) => {
  try {
    ...
    if (data.user.picture === 'not-found' || photoBlob.size > 0) {
      const uploadedImageUrl = await uploadBlobAsUserImage(photoBlob, oid, token);
